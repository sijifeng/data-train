...
class KafkaManager(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  def createDirectStream[K: ClassTag, V: ClassTag, KD <: Decoder[K] : ClassTag, VD <: Decoder[V] : ClassTag, R: ClassTag](
    ...
    val messages = {
      ...
      KafkaUtils.createDirectStream[K, V, KD, VD, R](
        ssc, kafkaParams, consumerOffsets, messageHandler)
    }
    ...
    topics.foreach(topic => {
      ...
      if (hasConsumed) {
        ...
        /**
          ...
          * 这时把consumerOffsets更新为earliestLeaderOffsets,还需有待验证。现在不能100%确定
          */
  ...
  /**
    ...
    * @param offset
    */
  def updateZKOffsets(brokers: String, groupId: String, topic: String, partition: Int, offset: Long): Unit = {
    ...
    val o = kafkaCluster.setConsumerOffsets(groupId, Map((topicAndPartition, offset)))
    if (o.isLeft) {
      println(s"Error updating the offset to Kafka cluster: ${o.left.get}")
    }
